...
public:
	...
	*/
	int outputVoxels(string filename);
...
protected:
	...
	*	2: otw.
	*/
	virtual unsigned int lower_check(Point lp, Point hp, int &nImagesConsistent) = 0; 
	virtual unsigned int upper_check(Point lp, Point hp, int &nImagesConsistent) = 0; 
	...
	*	hp:			high point
	*/
	void outputVoxels(ofstream &myfile, int cur, Point* lp, Point* hp, int &nVoxels);
...
protected:
	...
	int *_reliability;			// #. images consistent with
